interface DailyLockNoticeProps {
  batchName: string
}

export function DailyLockNotice({ batchName }: DailyLockNoticeProps) {
  return (
    <div
      className="flex gap-3 rounded-xl border px-4 py-3"
      style={{
        backgroundColor: '#fffbeb',
        borderColor: '#fcd34d',
      }}
    >
      <svg className="mt-0.5 h-5 w-5 shrink-0" style={{ color: '#d97706' }} fill="currentColor" viewBox="0 0 20 20">
        <path
          fillRule="evenodd"
          d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
          clipRule="evenodd"
        />
      </svg>
      <div>
        <p className="text-sm font-semibold" style={{ color: '#92400e' }}>
          Daily batch limit reached
        </p>
        <p className="mt-1 text-sm leading-relaxed" style={{ color: '#b45309' }}>
          <strong>{batchName}</strong> is running today. Another batch cannot be triggered until tomorrow.
        </p>
      </div>
    </div>
  )
}
